...
> **포크 PR 안내:** 검사가 표시되지 않으면 첫 워크플로 실행이 maintainer 승인을 기다리며 CI가 시작되지 않은 상태일 수 있습니다. 변경이 잘못된 것은 아닙니다. 승인 시점은 maintainer 확인 상황에 따라 다르며, 검사가 계속 시작되지 않으면 검사가 시작되지 않았다는 댓글을 남겨 주세요.
