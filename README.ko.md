...
의존성 scanner 리포트에서 근거를 설명할 수 있는 다음 행동을 정리하고 수용한 결정을
기록합니다. [제품](./PRODUCT.ko.md) · [로드맵과 현재 상태](./docs/ROADMAP.ko.md)

로컬에서 판정하며 runtime LLM·telemetry는 없습니다. 실행하는 scanner는 네트워크와
저장소 설정을 사용할 수 있습니다. 로컬 실행이 sandbox라는 뜻은 아닙니다.
...
> 이 저장소 자신에게 도구를 돌리는 것까지 포함합니다. 기능은 발전 중이며,
> [공개한 종료 코드와 JSON 계약](./docs/STABILITY.md)은 계속 적용됩니다.
...
그중 하나가 이번에 안 돌았으면 어느 것인지 말해 줍니다. JSON에서는
`missingSources`가 같은 조건을 전달합니다. 전부 다시 돌았으면 없는 의심을 지어내지
않습니다.

## 설치

zero-shelter를 설치한 뒤 프로젝트에 맞는 scanner 입력을 선택합니다. OSV-Scanner는
npm/pnpm audit에 근거를 추가할 수 있습니다. 이 도구의 yarn 실시간 수집에는
OSV-Scanner가 필요합니다.
...
지원하는 npm 프로젝트에서는 `npm audit`을 사용합니다. pnpm과 yarn은 아래와 같이
다릅니다. 읽을 수 있는 소스 하나로도 순위·baseline·지원되는 조치 안내를 제공합니다.
소스 간 병합에는 여러 소스의 기여가 필요하지만, 단일 소스 실행에도 가치가 있습니다.
...
지원하는 npm/pnpm 실행은 `osv-scanner` 없이도 판정할 수 있으며 건너뛴 scanner를
알립니다. 실제 기여한 근거를 기준으로 결과를 해석합니다.
