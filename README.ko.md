...
결과가 Security 탭에 올라가고 PR에 주석으로 붙습니다. **각 알림이 무엇을 해야 하는지
같이 실어 옵니다** — 직접 의존성이면 `npm i lodash@4.18.1`, 간접 의존성이면 그걸
...
여기엔 baseline을 기록해 두지 않았습니다. 우리 빌드를 초록으로 유지하려고 우리
도구를 침묵시키는 건, 이 프로젝트가 반대하려고 존재하는 바로 그 행동입니다.
